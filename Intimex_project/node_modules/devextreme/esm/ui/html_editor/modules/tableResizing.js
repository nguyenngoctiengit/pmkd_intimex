...
export default class TableResizingModule extends BaseModule {
    ...
    _fixTablesWidths($tables) {
        each($tables, (index, table) => {
            ...
            if ($columnElements.eq(0).attr("width")) {
                ...
                var tableWidth = null !== (_this$_tableLastWidth = this._tableLastWidth(frame)) && void 0 !== _this$_tableLastWidth ? _this$_tableLastWidth : $table.outerWidth();
                if (frame) {
    ...
    _getLineElements($table, index, direction) {
        var result;
        if ("vertical" !== direction) {
            result = $table.find("th:nth-child(".concat(1 + index, "), td:nth-child(").concat(1 + index, ")"))
        } else {
    ...
    _horizontalDragHandler(_ref2) {
        ...
        var realWidthDiff = $($lineElements.eq(0)).outerWidth() - currentLineNewSize;
    ...
    _fixColumnsWidth($table) {
        ...
        each(determinantElements, (index, element) => {
            var columnWidth = $(element).outerWidth();
            var $lineElements = this._getLineElements($table, index);
    ...
    _getColumnElementsSum(columnElements) {
        ...
        each(columnElements, (index, element) => {
            var $element = $(element);
            var columnWidth = this._getWidthAttrValue($element) || $element.outerWidth();
    ...
    option(option, value) {
        ...
        } else if ("minColumnWidth" === option || "minRowHeight" === option) {
